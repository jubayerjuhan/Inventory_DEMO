...
export const editUserProfile = (editDataForm) => async (dispatch) => {
  try {
    if (localStorage.token) {
      ...
      const { data } = await client.put(`/update-userprofile`, editDataForm)
      if (data.success) {
...
export const changePassword = (password) => async (dispatch) => {
  try {
    ...
    const { data } = await client.put(`/change-password`, password)
...
export const resetPassword = (passwords, token) => async (dispatch) => {
  try {
    ...
    const { data } = await client.put(`/reset-password/${token}`, passwords)
    // saveLocalStorage('token', data.token, 2)
...
export const deleteUser = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await client.delete(`/admin/delete/user/${id}`)
...
export const getSingleUser = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await client.get(`/admin/user/${id}`)
...
export const updateRole = (id, role) => async (dispatch) => {
  try {
    ...
    const { data } = await client.put(`//admin/user/edit-role/${id}`, { role })
